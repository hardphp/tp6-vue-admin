<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="seo关键词" width="200px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.seo_title }}</span>
        </template>
      </el-table-column>
      <el-table-column label="seo描述" width="250px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.seo_dec }}</span>
        </template>
      </el-table-column>
    ...
    <!-- 表单 -->
    <columnForm ref="fromColumn" :column-list="list" @updateRow="updateRow" />
...
export default {
  name: 'Column',
  components: { columnForm, treeTable },
  directives: {
    waves
  },
  filters: {
  ...
  methods: {
    ...
    handleCreate() {
      this.$refs.fromColumn.handleCreate()
    },
    handleUpdate(index, id) {
      this.$refs.fromColumn.handleUpdate(id)
    },
